...
class tspParser(object):
    def parse(self, problem, debug=False):
        self.is_debug = debug
        dm = TspManager(problem['capacity'])
        dm.nodes.append(TspNode(x=problem['depot'][0], y=problem['depot'][1], demand=0, px=problem['depot'][0],
                                py=problem['depot'][1], capacity=problem['capacity'], dis=0.0))
        for customer in problem['customers']:
            dm.nodes.append(TspNode(x=customer['position'][0], y=customer['position'][1], demand=customer['demand'],
                                    px=customer['position'][0], py=customer['position'][1], capacity=problem['capacity'], dis=0.0))
