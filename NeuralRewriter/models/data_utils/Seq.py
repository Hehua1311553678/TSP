...
class TspManager(SeqManager):
    ...
    def clone(self):
        ...
        for i, node in enumerate(self.nodes):
            res.nodes.append(
                TspNode(x=node.x, y=node.y, demand=node.demand, px=node.px, py=node.py, capacity=node.capacity,
    ...
    def add_route_node(self, node_idx):
        ...
        new_node = TspNode(x=node.x, y=node.y, demand=node.demand, px=pre_node.x, py=pre_node.y, capacity=pre_capacity, dis=cur_dis)
